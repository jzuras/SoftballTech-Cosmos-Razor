...
namespace Sbt.Data
{
    public class DemoContext : DbContext
    {
        ...
        public DbSet<Sbt.DataOld.DivisionsOld> Divisions { get; set; } = default!;
        public DbSet<Sbt.DataOld.SchedulesOld> Schedules { get; set; } = default!;
        public DbSet<Sbt.StandingsOld> Standings { get; set; } = default!;

        #region Data Access Layer Methods
        public async Task<LoadScheduleResult> LoadScheduleFileAsyncOld(IFormFile scheduleFile, string organization, string divisionID, 
            bool usesDoubleHeaders, bool clearScheduleFirst)
        {
            ...
            try
            {
                ...
                while (lines[lineNumber].Length > 0)
                {
                    ...
                    var standingsRow = new Sbt.StandingsOld
                    {
                ...
                for (int index = lineNumber + 1; index < lines.Count; index++)
                {
                    ...
                    var scheduleRow = new Sbt.DataOld.SchedulesOld
                    {
                        Organization = organization,
                        GameID = maxGameID++,
                        Day = gameDate,
                    ...
                    if (usesDoubleHeaders)
                    {
                        ...
                        scheduleRow = new Sbt.DataOld.SchedulesOld
                        {
                            Organization = organization,
                            GameID = maxGameID++,
                            Day = gameDate,
